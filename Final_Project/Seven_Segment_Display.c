...
{
	// Enable Clock to Port B (Bit 1)
	SYSCTL->RCGCGPIO |= 0x02;

	// Enable Clock to Port C (Bit 2)
	SYSCTL->RCGCGPIO |= 0x04;

	// Enable Clock to SSI2 (Bit 2)
	SYSCTL->RCGCSSI |= 0x04;

	// Configure PB4 (SSI2 CLK) and PB7 (SSI2 TX Data) for alternate function
	GPIOB->AFSEL |= 0x90;
	...
	// Set the direction of the PC7 pin as an output by
	// setting Bit 7 of the DIR register for Port C
	// Note: In this driver, PC7 will be used as the
	// SSI2 Slave Select pin (SSI2 SS) and it will be active low
	GPIOC->DIR |= 0x80;
	
	// Configure the PC7 pin to function as a GPIO pin by
	// clearing Bit 7 of the AFSEL register for Port C
	GPIOC->AFSEL &= ~0x80;

	// Enable Digital Functionality for PC7 by setting
	// Bit 7 of the DEN register for Port C
	GPIOC->DEN |= 0x80;

	// Initialize the output of the PC7 pin to high
	// by setting Bit 7 of the DATA register for Port C
	GPIOC->DATA |= 0x80;
...
void Seven_Segment_Display(int count_value)
{
	...
	if (count_value == 0)
	{
		...
		// Exit the function using the return statement
		return;
	}
	...
	for (uint8_t i = 1; i < (num_digits * 2); i = i * 2)
	{

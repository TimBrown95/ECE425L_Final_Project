...
void PMOD_BTN_Interrupt_Init(void(*task)(uint8_t pmod_btn_state))
{
	...
	// Clear the INTA field (Bits 7 to 5) of the IP[0] register (PRI0).
	NVIC->IP[0] &= ~0x000000E0;
	
	// Set the priority level to 3 for the GPIO Port A interrupts.
	// In the Interrupt 0-3 Priority (PRI0) register,
	// the INTA field (Bits  7 to 5) corresponds to Interrupt Request (IRQ) 0.
	// GPIO Port A has an IRQ of 0.
	NVIC->IP[0] |= (3 << 5);
...
void GPIOA_Handler(void)
{
	...
	if (GPIOA->MIS & 0x3C)
	{
		...
		// status of the PMOD BTN push buttons

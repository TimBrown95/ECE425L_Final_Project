...
void EduBase_Button_Interrupt_Init(void(*task)(uint8_t edubase_button_status))
{
	...
	// Configure the PD3 and PD2 pins as input
	// by clearing Bits 3 to 2 in the DIR register
	GPIOD->DIR &= ~0x0C;
	...
	// Allow the interrupts that are generated by the PD3 and PD2 pins
	// to be sent to the interrupt controller by setting
	// Bits 3 to 2 in the IM register
	GPIOD->IM |= 0x0C;
	
	// Clear the INTD field (Bits 31 to 29) of the IP[0] register (PRI0).
	NVIC->IP[0] &= ~0xE0000000;
	
	// Set the priority level to 3 for the GPIO Port D interrupts.
	// In the Interrupt 0-3 Priority (PRI0) register,
	// the INTD field (Bits 31 to 29) corresponds to Interrupt Request (IRQ) 3.
	// GPIO Port D has an IRQ of 3.
	NVIC->IP[0] |= (3 << 29);
...
void GPIOD_Handler(void)
{
	...
	if (GPIOD->MIS & 0x0C)
	{
		...
		// Acknowledge the interrupt from any of the 
		// following pins and clear it: PD3 and PD2
		GPIOD->ICR |= 0x0C;
	}
}
